...
namespace DigitalOpus.MB.Core{
	...
	public class MB3_MeshBakerEditorInternal{
        ...
        private static GUIContent
			...
			gc_objectsToCombineGUIContent = new GUIContent("Custom List Of Objects To Be Combined","You can add objects here that were not on the list in the MB3_TextureBaker as long as they use a material that is in the Material Bake Results"),
			gc_textureBakeResultsGUIContent = new GUIContent("Material Bake Result","When materials are combined a MB2_TextureBakeResult Asset is generated. Drag that Asset to this field to use the combined material."),
			...
			try{
				if (mom.textureBakeResults == null){
					if (_OkToCreateDummyTextureBakeResult(mom)){
						...
						mom.textureBakeResults = MB2_TextureBakeResults.CreateForMaterialsOnRenderer(gos.ToArray());
					}
				}
