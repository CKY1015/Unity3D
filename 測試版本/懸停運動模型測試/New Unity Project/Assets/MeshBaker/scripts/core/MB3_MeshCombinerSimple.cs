        ...
        void _initialize()
        {
            ...
            if (instance2Combined_MapCount() != objectsInCombinedMesh.Count)
            {
                ...
                for (int i = 0; i < mbDynamicObjectsInCombinedMesh.Count; i++)
                {
                    ...
                    {
						//Objects in objectsInCombinedMesh may have been destroyed so we can't trust it
						//however the instanceIDs can only be trusted within the context of a single play
						//if a scene is closed ad re-opened objects get different instance IDs
						//try to get instanceIDs from objectsInCombinedMesh if they exist otherwise use mbDynamicObjectsInCombinedMesh
