...
for(i = 0; i < ODODOOOCQC.roadObjects.Length; i++){
comboValues.Add(ODODOOOCQC.roadObjects[i]);
}
...
ODODOOOCQC.ODOQOCQOQO.Add(thisso);
}
}
}

ODODOOOCQC.roadObjects = comboValues.ToArray();
ODODOOOCQC.OQDDDDDCDQ();
...
for(int i = 0 ; i < ODODOOOCQC.ODOQOCQOQO.Count;i++){
ODODDQQO so = (ODODDQQO)ODODOOOCQC.ODOQOCQOQO[i];
