...
public bool OCQOQQCDCQ;
public bool ODQDOQOO;
public float OQCCQQCCQQ;
public float ODOOQQOO;
public Transform[] OCQOQDDQCOs;
public float[] trperc;
...
public string OCCCCOCOOO = "0";
public string OQDCOQQQOQ = "0";
...
if(!objectScript.OQQDQQCCDQ){  


Vector3 v;
if(snapMarker){
if(OOODCCCCOD.terrain != null){
v = transform.position;
v.y = OOODCCCCOD.terrain.SampleHeight(v);
transform.position = v;
}
}

Vector3 change = transform.position - oldPos;
if(OCQOQQCDCQ && oldPos != Vector3.zero && change != Vector3.zero){
int i = 0;
foreach(Transform tr in OCQOQDDQCOs){
...
changed = true;
if(objectScript.OQQDQQCCDQ){
objectScript.OQQDCODQCD.specialRoadMaterial = true;
}
}
...
if(objectScript.OQQDCODQCD == null){

List<ODODDQQO> arr = OQOQQQDDOD.OOOQQDOOCQ(false);
objectScript.OQODDOQDDC(arr, OQOQQQDDOD.OCDODCQODQ(arr), OQOQQQDDOD.OCDOCQODDC(arr));
...
for(int i = 0; i < objectScript.OCQOQDDQCOs.Length; i++){
if(objectScript.OCQOQDDQCOs[i] == this.gameObject)return true;
}
return false;
}
}
