...
public class UI : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        ...
        if (s == true)
        {
            ...
            transform.rotation = Quaternion.Euler(-(float)TCP_Handler.pitch, transform.localEulerAngles.y-(float)TCP_Handler.yaw, -(float)TCP_Handler.roll);
    ...
    void OnGUI()
    {
        ...
        GUI.Box(new Rect(10, 10, 200, 500), "Button");
            ...
            if (SocketApp.TCP_clientList.Count == 0 && open == true)//確定3顆都連線
            {
                GUI.color = Color.green;
